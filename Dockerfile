...
COPY ./apache-maven-3.6.0-bin.tar.gz /d/apache-maven-3.6.0-bin.tar.gz
...
RUN mv /etc/apt/sources.list /etc/apt/sources.list.bak && \
	...
	cd /d/ && \
	mkdir /usr/maven/ && \
	cp apache-maven-3.6.0-bin.tar.gz /usr/maven/ && \
	cd /usr/maven/ && \
	tar -xvf apache-maven-3.6.0-bin.tar.gz && \
	cd apache-maven-3.6.0/ && \
	rm -rf /usr/maven/apache-maven-3.6.0-bin.tar.gz && \
	rm -rf /d/

ENV MAVEN_HOME=/usr/maven/apache-maven-3.6.0
ENV JAVA_HOME=/usr/java
ENV JRE_HOME=/usr/java/jre
ENV CLASSPATH=.:$JAVA_HOME/lib:$JRE_HOME/lib:$CLASSPATH
ENV PATH=$JAVA_HOME/bin:$JRE_HOME/bin:$PATH:$MAVEN_HOME/bin:$PATH
